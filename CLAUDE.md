...
- 2026-07-19 (FLOWCHART/ALUR): Dibuat laporan alur di `docs/`: `ALUR-WEBSITE.md`, `ALUR-WEBSITE.pdf`, `alur-website.html`, + gambar `docs/images/flow-umum.png` & `flow-permohonan.png`. Dua flowchart Mermaid sesuai kaidah (terminator/proses/keputusan/IO): (1) Alur Umum Website, (2) Alur Permohonan Bahan. Cara buat ulang (scratchpad): `mermaid.min.js` (di-curl dari jsdelivr), `flow-umum.html`/`flow-permohonan.html` (mermaid inline), `render-flows.js` (CDP screenshot SVG → PNG scale 2), lalu `build-guide.js <md> <html>` + chrome print-to-pdf. `build-guide.js` kini terima argv (md in, html out).

- 2026-07-19 (USER GUIDE): Dibuat panduan pengguna di folder `docs/`: `PANDUAN-PENGGUNA.md` (sumber teks), `PANDUAN-PENGGUNA.pdf` (2.4MB), `panduan.html` (self-contained, gambar base64), + 8 screenshot asli di `docs/images/` (login, dashboard, stok bahan, lab, permohonan, laporan pengadaan, riwayat, kelola pengguna). Cara buat ulang (semua di scratchpad, tanpa dependency): (1) `shots.js` = driver CDP (Chrome headless + WebSocket bawaan Node 22) login admin lalu screenshot tiap halaman via klik teks sidebar; (2) `build-guide.js` = konverter MD→HTML mini (heading/tabel/list/blockquote/img base64) + CSS cetak; (3) PDF via `chrome --headless --no-pdf-header-footer --print-to-pdf`. Port dev saat itu 3002. BELUM di-commit (binary besar; tanya user dulu).

- 2026-07-18 (DEPLOY + isi stok reset SQL): Semua perubahan sesi ini di-PUSH ke `main` (2 commit: fitur UI/audit + update SQL) → live di Netlify. `reset-and-seed-bkb.sql` diperbarui: stok tidak ada yang 0 (persediaan 0 diisi penuh = target) + `min_stock` ~20% target (>=1). User akan Run SQL ini di Supabase untuk serah-terima client (audit+permohonan kosong, 28 bahan stok terisi). Catatan: `reverseTransaction` & fitur tabel BHP baru BELUM diuji E2E di DB (diverifikasi via build).

- 2026-07-18 (hapus halaman LowStock, alihkan notif kritis): Halaman **LowStock DIHAPUS** (`src/pages/LowStock.jsx` dihapus permanen; route & import di App.jsx dibuang). Semua pemicu notif kritis (lonceng Navbar, kartu & tombol "Tinjau Stok Kritis" di Dashboard) masih memanggil `onNavigate('low-stock')`, TAPI App.jsx kini punya wrapper `handleNavigate`: `'low-stock'` → buka **Stok Bahan dengan filter kritis otomatis** (`Materials` terima prop `initialStockFilter`, useEffect set `stockStatusFilter`); navigasi lain reset filter itu ke null. Alasan filter penting: tabel BHP kini dikelompokkan/collapse, jadi tanpa filter bahan kritis bisa tersembunyi di grup tertutup. Build lulus (konfirmasi tak ada importer lain LowStock). BELUM di-push.

- 2026-07-18 (koreksi audit + hapus menu stok kritis): (a) **Riwayat Audit dapat tombol "Batalkan"** pada entri stok masuk/keluar (`reverseTransaction` di storage.js + modal alasan di History.jsx, admin-only). Ini IMPLEMENTASI dari aturan "koreksi lewat entri penyesuaian" — baris asli TIDAK dihapus, stok dikembalikan (in→kurangi, out→tambah), pembatalan dicatat entri koreksi baru bertanda `[koreksi:<txid>]`. Guard: cegah pembatalan ganda (cek marker), tolak entri adjustment, stok tak cukup → gagal (mis. barang restock sudah terpakai). Aturan immutable TETAP: tetap TIDAK ada hard-delete di audit. (b) Menu **"Peringatan Stok Kritis" dihapus dari sidebar** (permintaan client, dianggap ganggu; peringatan kritis tetap ada di halaman Stok Bahan). Route `low-stock` & LowStock.jsx dibiarkan (tak terpakai dari sidebar). (c) Konfirmasi: tombol "Tandai Dibeli" di Laporan Pengadaan MEMANG sudah auto-nambah stok (`recordIncomingStock`) — tak ada perubahan. Build lulus, BELUM di-push.

- 2026-07-18 (tabel BHP: spesifikasi + kelompok per nama): (a) Spesifikasi kini tampil di bawah nama bahan di tabel + ikut dicari (filter search sebelumnya cuma nama+MK). (b) **Tabel dikelompokkan per nama** (Materials.jsx): nama sama (mis. Nachi, Rivet) muncul SEKALI sebagai baris induk yang bisa dibuka-tutup (chevron), menampilkan "N spesifikasi" + total stok + status "ADA KRITIS"; klik → sub-baris tiap spesifikasi dengan stok/status/aksi sendiri. Grup 1-item tampil biasa. State `expandedGroups` (Set), default tertutup. (c) Tombol **"Simpan & Tambah Lagi"** di form tambah bahan: nama/lab/semester/satuan dipertahankan, spesifikasi+stok dikosongkan → input banyak varian tanpa retype nama. CATATAN penting: tiap spesifikasi TETAP baris sendiri di DB (stok/min/target beda per varian) — perubahan ini murni tampilan, DB tak berubah. Build lulus, BELUM di-push.

- 2026-07-18 (persiapan serah-terima klien): (1) Underline pada judul surat ("SURAT PERMOHONAN & PEMAKAIAN / BAHAN HABIS PAKAI") dihapus (2 aturan `text-decoration: underline` di `letter.js` dibuang). (2) Dibuat file **`reset-and-seed-bkb.sql`** (dijalankan SEKALI di Supabase SQL Editor): hapus semua `transactions` (audit) + `requests` + `materials`, lalu isi **28 BHP Bengkel Kerja Bangku dan Plat (lab-02)** dari `Usulan BHP BENGKEL KERJA BANGKU 2026.xlsx`. Mapping: `stock` = kolom Persediaan, `target_stock` = Persediaan + Usulan (biar Laporan Pengadaan "Perlu Dibeli" = jumlah Usulan), `unit` diambil dari kolom Persediaan (hindari typo "buag"), `semester`/`min_stock` default. Belum dijalankan oleh user.

- 2026-07-18 (fix pesan hapus BHP): `deleteMaterial` menangkap error FK Postgres (`23503`) dan menampilkan pesan Indonesia jelas ("Bahan tidak bisa dihapus karena sudah punya riwayat permohonan/transaksi"), bukan error DB mentah. Bahan yang sudah dipakai di `requests` memang tidak boleh dihapus (audit immutable) — konsisten dgn aturan akun berhistori.
